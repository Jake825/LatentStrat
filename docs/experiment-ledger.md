...
| Physics-consistent multitask smoke | `artifacts/smoke/2026-static-multitask-physics-v2/` | V5.6.4 prior and cached 2026 breakdown corpus | `8` development + `8` refit leaves x `2` epochs, seed `2026` | Validate atomic physics targets, fixed multitask arms, exact resume, artifacts, and simple TensorBoard telemetry | Pending final smoke acceptance; no scientific claim | Formal training remains blocked until the complete smoke dashboard and timing gate pass |
| Physics-consistent multitask study | `artifacts/reference/2026-static-multitask-physics/` | V5.6.4 prior and cached 2026 breakdown corpus | Planned: `24` development trajectories x `100` epochs plus `24` selected-duration refits | Test whether dense targets and exact score consistency rescue Full-match relative to Additive | Not yet run | The generated report, not this planned row, will own the result |
