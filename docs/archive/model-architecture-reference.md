...
### Match-Breakdown V2 Ablation

The opt-in V2 artifact preserves the V1 encoder and decoder dimensions and adds a diagnostic scalar
quality head:

```text
S_alliance: [B, 16]
quality_head: Linear(16, 1)
```

The quality head is saved for reproducibility and ranking diagnostics. It is not part of the frozen
downstream target contract; exported targets remain `S_alliance`.

V2 training stages the auxiliary losses:

```text
epoch 1:  L_reconstruction
epoch 5:  + 0.10 * MSE(S_corrupt, stopgrad(S_full))
epoch 10: + 0.02 * MarginRankingLoss(q(S_red), q(S_blue))
epoch 15: + 0.05 * L_score_equation_residual
```

The denoising pass hides 10 percent of observed source fields while preserving the raw physical-zero
contract. Score-equation rules use numeric raw-scale decoder outputs only. Startup audits promote
passing candidates and abort if a required equation drifts below its configured pass threshold.
