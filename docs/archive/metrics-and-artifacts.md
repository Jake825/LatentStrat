...
`artifacts/pretraining/match-breakdown/v1_2015_2026/` and writes:
...
The explicit V2 structured ablation writes beside V1 under
`artifacts/pretraining/match-breakdown/v2_2015_2026/`. It adds:

| File | Meaning |
|---|---|
| `rule_audit.json` | eval-split and all-data score-equation audits with schema and rule hashes |
| `resolved_config.json` | exact V2 objective settings |
| V2 `validation_report.json` sections | denoising MSE, quality ranking diagnostics, score/foul correlations, and decoder residuals |
| V2 `training_history.csv` columns | reconstruction, denoising, ranking, consistency, total loss, and pre-clip gradient norm |

The V2 quality head is diagnostic-only. Frozen downstream targets remain the 16D alliance vectors.
V2 remains `promotion_eligible=false` until reviewed against V1.
...
| optional `baseline_neighbor_drift.csv` and `baseline_score_sorting_drift.csv` | keyed V1/V2 review diagnostics when `--baseline-artifact-dir` is supplied |
