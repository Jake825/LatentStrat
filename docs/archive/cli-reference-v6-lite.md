...
When paths are omitted, CLI defaults use the canonical generated-file layout: `data/cache/` for TBA, Statbotics, and OpenAI SQLite caches; `data/pretraining/match-breakdown/corpus.sqlite` for the durable raw match-breakdown corpus; `data/scouting/scouting.db` for scouting; `data/embeddings/latentstrat_embeddings.sqlite` for durable consolidated embeddings; `data/features/prior/`, `data/features/season/`, `data/features/event/`, and `data/features/pretraining/match-breakdown/` for Parquet feature tables; `data/sidecars/` for relational sidecars; grouped `artifacts/` subdirectories for model outputs; and `runs/` for TensorBoard. Explicit older paths still work when provided.
...
latentstrat sync-match-breakdowns `
  --start-season 2015 `
  --end-season 2026 `
  --corpus-db data/pretraining/match-breakdown/corpus.sqlite
```
...
offline historical score artifact. Without `--config`, this retains the reconstruction-only V1
baseline:
...
Use the explicit V2 structured-objective ablation config to add latent denoising, winner ranking,
and audit-first numeric score-equation consistency:
...
Explicit CLI options such as `--epochs` override the YAML value. V2 writes
`artifacts/pretraining/match-breakdown/v2_2015_2026/` and does not replace V1.
...
When inspecting V2, pass
`--baseline-artifact-dir artifacts/pretraining/match-breakdown/v1_2015_2026` to write keyed
nearest-neighbor and score-sorting drift tables against V1.
