"""Offline frozen target spaces for LatentStrat V6-Lite."""
...
from latentstrat.pretrain_features import EmbeddingStats, embed_narratives

PARQUET_ENGINE = "pyarrow"
OFFICIAL_EVENT_TYPES = frozenset(range(8))
SCORE_INTEGRATION_PENDING = (
    "score_embedding is offline-only in V6-Lite V1. Build and validate the historical "
...
def load_world_model_options(path: str | Path) -> WorldModelOptions:
    ...
    world_model = payload.get("world_model", payload)
    return WorldModelOptions.model_validate(world_model)
...
def load_world_model_bundle(path: str | Path) -> LoadedWorldModelBundle:
    ...
    options = WorldModelOptions.model_validate(payload["world_model"])
    spaces = {
