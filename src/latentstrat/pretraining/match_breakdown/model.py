...
class MatchBreakdownAutoencoder(nn.Module):
    ...
    def __init__(
        self,
        season_widths: dict[int, int],
        latent_dim: int = 16,
        *,
        quality_head_enabled: bool = False,
    ) -> None:
        super().__init__()
        ...
        self.quality_head_enabled = quality_head_enabled
        ...
        self.quality_head = nn.Linear(latent_dim, 1) if quality_head_enabled else None

    def _validate_inputs(self, season: int, values: Tensor, mask: Tensor) -> str:
        key = str(int(season))
    ...
    def encode(self, season: int, values: Tensor, mask: Tensor) -> Tensor:
        key = self._validate_inputs(season, values, mask)
        return self.bottleneck(self.encoders[key](values, mask))

    def decode(self, season: int, latent: Tensor) -> Tensor:
        key = str(int(season))
        if key not in self.decoders:
            raise ValueError(f"No match-breakdown schema is registered for season {season}.")
        return self.decoders[key](latent)

    def quality(self, latent: Tensor) -> Tensor:
        if self.quality_head is None:
            raise ValueError("This match-breakdown checkpoint does not include a quality head.")
        return self.quality_head(latent).squeeze(-1)

    def forward(self, season: int, values: Tensor, mask: Tensor) -> tuple[Tensor, Tensor]:
        latent = self.encode(season, values, mask)
        return self.decode(season, latent), latent
