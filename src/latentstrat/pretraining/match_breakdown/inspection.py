...
from latentstrat.artifacts.checkpoints import load_match_breakdown_checkpoint_model
...
from latentstrat.pretraining.match_breakdown.train import _season_tensors, _split_indices
...
BASELINE_COMPARISON_FILES = (
    "baseline_neighbor_drift.csv",
    "baseline_score_sorting_drift.csv",
)
...
def load_match_breakdown_model(
    ...
    """Reload through the centralized artifact factory."""

    return load_match_breakdown_checkpoint_model(path)
...
def _linear_probe_r2(matrix: np.ndarray, target: np.ndarray) -> float | None:
    if len(matrix) < 2 or np.allclose(target, target[0]):
        return None
    design = np.column_stack([np.ones(len(matrix)), matrix])
    coefficients, *_ = np.linalg.lstsq(design, target, rcond=None)
    prediction = design @ coefficients
    total = float(np.sum((target - target.mean()) ** 2))
    return float(1 - np.sum((target - prediction) ** 2) / total) if total > 0 else None
...
def write_baseline_comparison(
    ...
    *,
    ...
    """Write sampled keyed V1/V2 neighbor and score-sorting drift diagnostics."""

    baseline_path = Path(baseline_artifact_dir) / "embeddings.parquet"
    if not baseline_path.exists():
        raise FileNotFoundError(f"Baseline embeddings do not exist: {baseline_path}")
    baseline = pd.read_parquet(baseline_path, engine=PARQUET_ENGINE)
    if set(candidate["row_id"]) != set(baseline["row_id"]):
        raise ValueError("Candidate and baseline embeddings must contain identical row IDs.")
    candidate = candidate.sort_values("row_id").reset_index(drop=True)
    baseline = baseline.set_index("row_id").loc[candidate["row_id"]].reset_index()
    rng = np.random.default_rng(seed)
    sample_indices = np.sort(
        rng.choice(len(candidate), size=min(max_rows, len(candidate)), replace=False)
    )
    candidate_sample = candidate.iloc[sample_indices].reset_index(drop=True)
    baseline_sample = baseline.iloc[sample_indices].reset_index(drop=True)
    candidate_matrix = candidate_sample[list(LATENT_COLUMNS)].to_numpy(dtype=float)
    baseline_matrix = baseline_sample[list(LATENT_COLUMNS)].to_numpy(dtype=float)

    def nearest(matrix: np.ndarray) -> np.ndarray:
        if len(matrix) < 2:
            raise ValueError("Baseline comparison requires at least two sampled embeddings.")
        count = min(10, len(matrix))
        distances, indices = NearestNeighbors(n_neighbors=count).fit(matrix).kneighbors(matrix)
        result = []
        for row, (_, neighbors) in enumerate(zip(distances, indices, strict=True)):
            result.append(next(int(index) for index in neighbors if int(index) != row))
        return np.asarray(result, dtype=int)

    candidate_neighbors = nearest(candidate_matrix)
    baseline_neighbors = nearest(baseline_matrix)
    score = candidate_sample["alliance_score"].to_numpy(dtype=float)
    neighbor_drift = pd.DataFrame(
        {
            "row_id": candidate_sample["row_id"],
            "baseline_neighbor_row_id": baseline_sample.iloc[baseline_neighbors][
                "row_id"
            ].to_numpy(),
            "candidate_neighbor_row_id": candidate_sample.iloc[candidate_neighbors][
                "row_id"
            ].to_numpy(),
            "same_nearest_neighbor": baseline_neighbors == candidate_neighbors,
            "baseline_neighbor_score_gap": np.abs(score - score[baseline_neighbors]),
            "candidate_neighbor_score_gap": np.abs(score - score[candidate_neighbors]),
        }
    )
    score_drift = pd.DataFrame(
        [
            {
                "metric": "same_nearest_neighbor_rate",
                "baseline": 1.0,
                "candidate": float(neighbor_drift["same_nearest_neighbor"].mean()),
            },
            {
                "metric": "nearest_neighbor_score_gap_mean",
                "baseline": float(neighbor_drift["baseline_neighbor_score_gap"].mean()),
                "candidate": float(neighbor_drift["candidate_neighbor_score_gap"].mean()),
            },
            {
                "metric": "linear_score_probe_r2",
                "baseline": _linear_probe_r2(baseline_matrix, score),
                "candidate": _linear_probe_r2(candidate_matrix, score),
            },
        ]
    )
    score_drift["delta"] = score_drift["candidate"] - score_drift["baseline"]
    output = Path(output_dir)
    neighbor_drift.to_csv(output / "baseline_neighbor_drift.csv", index=False)
    score_drift.to_csv(output / "baseline_score_sorting_drift.csv", index=False)
    return {
        "note": "Review diagnostic only; not automatic promotion evidence.",
        "artifact_dir": str(baseline_artifact_dir),
        "embeddings_sha256": _sha256_file(baseline_path),
        "sample_rows": len(candidate_sample),
        "generated_files": {
            name: _sha256_file(output / name) for name in BASELINE_COMPARISON_FILES
        },
    }
...
def write_match_breakdown_inspection(
    ...
    *,
    ...
    alliance_features_path = resolve_migrated_path(bundle["alliance_features"]["path"])
    ...
    if baseline_artifact_dir is not None:
        baseline_comparison = write_baseline_comparison(
            production,
    ...
    if baseline_comparison is not None:
        generated.update(baseline_comparison["generated_files"])
    ...
    if baseline_comparison is not None:
        manifest["baseline_comparison"] = baseline_comparison
    _json_write(output / "inspection_manifest.json", manifest)
    write_artifact_manifest(
        output,
        workflow="pretraining.match-breakdown.inspect",
        artifact_version=str(bundle.get("artifact_version") or "v1"),
        resolved_config=asdict(options),
        source_files={
            "bundle": bundle_path,
            "embeddings": production_path,
            "model": model_path,
            "eval_model": eval_model_path,
            "alliance_features": alliance_features_path,
        },
        promotion_eligible=False,
        promotion_note=manifest["note"],
    )
